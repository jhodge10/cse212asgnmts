...
public static class ArraysTester {
    ...
    {
         static List<double> MultiplesOf(double number, int length)
        {
    ...
    private static void RotateListRight(List<int> data, int amount)
    {
        // TODO Problem 2 Start (don't forget to fill out the 01-prove-response.docx)
         static List<T> RotateListRight<T>(List<T> list, int rotationAmount)
        {
            if (list == null || rotationAmount <= 0 || rotationAmount >= list.Count)
            {
                throw new ArgumentException("Invalid input.");
            }

            int splitPoint = list.Count - rotationAmount;
            var firstSublist = list.Take(splitPoint).ToList();
            var secondSublist = list.Skip(splitPoint).Reverse().ToList();
            return firstSublist.Concat(secondSublist).ToList(); 
        }
    }
}